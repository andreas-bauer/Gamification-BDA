...
names(demographic) <- c("group", "studentId", "expJava", "expTest", "expWeb")
...
```{r}
withGame <- subset(withGameRaw, select = c(Group, Students.ID, Coverage.G, X..Interactions.G, X..Issue.G))

names(withGame) <- c("group", "studentId", "coverage", "widgetdInspected", "bugsIdentified")

withGame <- withGame %>% mutate(coverage = str_replace(coverage,",",".") %>% 
                      str_replace("%","") %>% 
                      as.numeric() %>% 
                      (function(x) x/100))
```
