...
```
A mean deviation of 3 for `rlnorm` results in a mean of 33 which is a good starting point for a predictor.

```{r}
N <- 100
curve(dlnorm(x,3,1), from = 0, to=N*1.5, n=N, xlab="N", ylab = "dlnorm(3, 1)",  col = "blue")
```
Of the three experience properties, experience with testing should have the highest impact, flowed by experience with web development and experience with Java: `ET > EW > EJ`
Therefore, priors for `EW` and `EJ` should be smaller.
```{r}
for (i in c(2, 2.2, 2.4, 2.6, 2.8)) {
  am <- mean(rlnorm(1000, i, 1))
  print(paste("meanlog", i, ":", round(am)))
}
```
...
for (i in 1:N) {
  curve(a[i] + b_T[i]*x + b_W[i]*x + b_J[i]*x,  col = "blue", add = TRUE)
}
```
...
m3 <- ulam(
  alist(widgetsInspected ~ dpois(lam),
    log(lam) <- b_T*expTest + b_W*expWeb + b_J*expJava,    
    b_T ~ dlnorm(3, 1),
    b_W ~ dlnorm(2.6, 1),
    b_J ~ dlnorm(2.4, 1)
